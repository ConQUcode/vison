...
- **数据段字节数（Len）**：`2`
- **注意事项**：Lower-controller execution status for discrete hardware commands and arm targets.
...
- **数据段字节数（Len）**：`3`
- **注意事项**：Discrete hardware command. Publish once; reliable transport handles delivery acknowledgement.

| task_id | Device | task_status = 0 | task_status = 1 |
| :---: | :--- | :--- | :--- |
| 0 | gripper | grip | open |
...
The lower controller reports execution with ExecutionCallback callback_id = 2.
...
- **数据段字节数（Len）**：`8`
- **注意事项**：Differential-drive command: linear_x in m/s and angular_z in rad/s.

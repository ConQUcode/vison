...
static uint8_t ARM_TOOL_DISABLED_UNUSED ArmToolSelfTest(void)
{
    ...
    if (!ArmToolGetCenterFromWrist(&wrist, 0.0f, 0.0f, &center) ||
        !ArmToolNear(center.x_mm, 130.0f, 0.001f) ||
    ...
    if (!ArmToolGetCenterFromWrist(&wrist, 0.0f, -90.0f, &center) ||
        ...
        !ArmToolNear(center.z_mm, 270.0f, 0.001f)) {
        fail_mask |= 1u << 6;
    }
    if (!ArmToolGetCenterFromWrist(&wrist, 0.0f, 90.0f, &center) ||
        !ArmToolNear(center.x_mm, 100.0f, 0.001f) ||
        !ArmToolNear(center.z_mm, 330.0f, 0.001f)) {
        fail_mask |= 1u << 7;
    }
    if (!ArmToolGetCenterFromWrist(&wrist, 90.0f, 0.0f, &center) ||
        !ArmToolNear(center.x_mm, 100.0f, 0.001f) ||
        !ArmToolNear(center.y_mm, 230.0f, 0.001f) ||
...
static void ARM_TOOL_DISABLED_UNUSED ArmToolRefreshServoFeedback(uint32_t now_ms)
{
    ...
    for (index = 0u; index < 2u; ++index) {
        ...
        if (HSLServoGetStatus(ids[index], &status) == 0u) {
        ...
        g_arm_tool_debug.servo_feedback_valid[index] =
            HSLServoFeedbackFresh(ids[index], now_ms);
                               ...
                               uint32_t now_ms)
{
    ...
    uint16_t target = feedback >
            (ARM_GRIPPER_SERVO_MIN_POS + ARM_GRIPPER_RELIEF_POS) ?
        (uint16_t)(feedback - ARM_GRIPPER_RELIEF_POS) :
...
static void ArmToolProcessClosing(uint32_t now_ms, uint8_t boot_motion)
{
    ...
    if (ArmToolGripperFeedbackArrived() != 0u) {
        if (boot_motion == 0u) {
            ...
            g_arm_tool_debug.gripper_target_state =
                ARM_GRIPPER_CLOSED_EMPTY;
        }
        return;
    }
    if (elapsed_ms >= ARM_GRIPPER_STALL_START_IGNORE_MS &&
        ArmToolUpdateStallWindow(now_ms) != 0u) {
        uint16_t close_travel =
            g_arm_tool_debug.gripper_feedback_pos >=
                    g_arm_tool_debug.gripper_close_start_pos ?
            (uint16_t)(g_arm_tool_debug.gripper_feedback_pos -
                       g_arm_tool_debug.gripper_close_start_pos) : 0u;
        ...
        if (boot_motion != 0u) {
        ...
        } else if (close_travel >=
                   ARM_GRIPPER_STALL_MIN_CLOSE_TRAVEL_POS) {
            ...
            g_arm_tool_debug.gripper_jam_count++;
            ArmToolStartRelief(ARM_GRIPPER_RELIEF_JAM, now_ms);
        }
        return;
    }
...
static void ArmToolProcessRelief(uint32_t now_ms)
{
    ...
    if (ArmToolGripperFeedbackArrived() == 0u) {
        if ((uint32_t)(now_ms -
                g_arm_tool_debug.gripper_action_start_tick) >
            ARM_GRIPPER_RELIEF_MOVE_TIME_MS +
            ...
            g_arm_tool_debug.init_state =
                g_arm_tool_debug.init_state == ARM_TOOL_INIT_RELIEVING ?
                ARM_TOOL_INIT_ERROR : g_arm_tool_debug.init_state;
        }
        return;
    }
...
void ArmToolInit(void)
{
    ...
    if (initialized != 0u &&
        HSLServoConfigureFeedbackPolling(
            feedback_ids, 2u, HSL_SERVO_DEFAULT_POLL_PERIOD_MS) == 0u) {
        initialized = 0u;
    }
    if (initialized != 0u &&
        HSLServoConfigureBoardVoltagePolling(
            HSL_SERVO_DEFAULT_VOLTAGE_POLL_PERIOD_MS) == 0u) {
        initialized = 0u;
    }

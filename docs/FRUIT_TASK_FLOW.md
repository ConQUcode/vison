...
先执行左侧 `[0,380,-150] -> [0,480,-150] mm`，再执行只对Y取负的
右侧 `[0,-380,-150] -> [0,-480,-150] mm`。到接近点的请求速度为
`600 mm/s`，最后 `100 mm` 推进段降为 `150 mm/s`；两侧分别复用当前
已验证的A左/A右放置profile，返回前方后持续交替。
...
当前默认 `APP_MODE_ARM_BD_OBSERVATION_TEST`。上电正常 HOME 后，
`APP_ARM_BD_OBSERVATION_ACTIVE_SIDE`当前选择RIGHT。两个已保存点为：

```text
LEFT： q1=+90 deg，夹爪中心=[0,+57,210] mm
RIGHT：q1=-90 deg，夹爪中心=[0,-57,210] mm
```

当前上传固件先同步移动底座、主臂和ID1进入右侧紧凑过渡姿态，再提交
一次夹爪中心直线命令：

```text
HOME关节=[0,90,-60] deg
同步过渡关节=[-90,120,-48] deg，ID1相对俯仰=-18 deg
过渡夹爪中心约=[0,-225.643,174.610] mm
整段工具中心最大|Y|=231.470 mm，限制=260 mm
最终夹爪中心坐标=[0,-57,210] mm
最终世界绝对俯仰=-30 deg
请求速度=150 mm/s
预期终点IK约=[-90.000,168.151,-47.928] deg
```

终点的ID1相对小臂俯仰约 `-66.079 deg`，在 `[-90,92.4] deg`
限位内。命令完成后保持位置，不闭合ID2、不运行底盘、不进入AC循环。

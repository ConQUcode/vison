...
释放 ID1 相对小臂均为 `-45 deg`。恢复转运姿态后分别经 `+90/-90`
返回 `q1=0`。
...
当前 `protocol.c/.h`、协议哈希和水果识别消息没有因底盘整理而修改。
未来上位机桥应调用同一个 `ChassisSubmitCommand/ChassisGetStatus` 边界。
水果识别目前只更新 `g_fruit_usb_debug`，不直接改变 A 区静态测试表。

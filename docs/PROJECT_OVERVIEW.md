...
命令/状态边界已为未来上位机预留，但当前 USB 协议尚未包含底盘命令。
...
- A左教导姿态：`q=[90,31.64,-111.21] deg`，ID1相对小臂 `-41.5 deg`。
- A右教导姿态：`q=[-90,33.88,-105.85] deg`，ID1相对小臂 `-41.5 deg`。

任务配置中的观察坐标只用于 Watch，不参与当前关节教导抓取；其 Y 符号
与上述左右业务名不一致，接入坐标规划前必须重新实机确认，不能据此改动
已验证的关节姿态和放置方向。

放置流程由显式 `App_Arm_Place_Profile_s` 决定。A左从左侧逆时针转到
后方，A右从右侧顺时针转到后方；ID2 张开后小臂从 `q3=-70` 上抬到
`q3=-80`，底座回到 `q1=0` 后才恢复 `[0,90,-100]` 转运姿态。底层
不再根据抓取关节角猜测侧别。
...
命令层之外增加低带宽外环，不能把里程计 `y_m` 当成外部绝对位置。
